...
import StorageClusters from 'components/Federation/StorageClusters.vue'
...
const divNodes = ref<typeof StorageClusters[]>([])
const filterOptions = [
  {
    label: '每30s刷新',
    labelEn: 'Refresh every 30 seconds',
...
onBeforeMount(async () => {
  ...
  console.log(monitorCephTable.value)
})
onUnmounted(() => {
  clearInterval(timer)
})
...
<template>
  <div class="StoragePage" style="min-width: 1000px">
    ...
    <storage-clusters v-for="(monitor, index) in monitorCephTable" :key="monitor.id" :unit-ceph="monitor" :ref="el=>{divNodes[index] = el}" @is-emit="childEmit"></storage-clusters>
  </div>
</template>
